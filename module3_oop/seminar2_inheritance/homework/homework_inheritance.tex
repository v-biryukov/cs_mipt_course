...
\subsection{ (*) Окна}
В файле \texttt{windows.cpp} написан класс \texttt{BaseWindow}, описывающий простейшее окно. Это окно состоит из двух прямоугольников. Первый прямоугольник определяет границы отрисовки окна, а второй определяет границы области, за которую окно можно перетаскивать. Создайте следующие классы:
\begin{itemize}

\item \texttt{ClosableWindow}, наследник \texttt{BaseWindow}.
У таких окон, помимо функционала \texttt{BaseWindow}, в правом верхнем углу должна быть кнопка, которая бы закрывала это окно. Используйте класс \texttt{Button} из файла \texttt{button.hpp} для создания кнопок.

\item \texttt{MessageWindow}, наследник \texttt{ClosableWindow}.
У объектов этого класса, помимо функционала \texttt{ClosableWindow} должен быть текст, в котором отображается некоторая строка. 

\item \texttt{QuestionWindow}, наследник \texttt{ClosableWindow}.
У объектов этого класса, помимо функционала \texttt{ClosableWindow} должен быть текст, в котором отображается некоторая строка. А также две кнопки внизу: Ok и Cancel. При нажатии на кнопку Ok в консоль должно выводиться строка \texttt{Ok}, а при нажатии на кнопку Cancel -- строка \texttt{Cancel}.


\item \texttt{ResizableWindow}, наследник \texttt{BaseWindow}.
Помимо функционала \texttt{BaseWindow}, размер таких окон должен быть изменяемым. Чтобы поменять размер окна нужно навести мышкой на край окна, нажать и потянуть. При наведении на край экрана, курсор мышки должен меняться.

\item \texttt{ShapeDrawWindow}, наследник \texttt{ResizableWindow}.
С помощью этого окна должно быть возможно рисовать на фоне разные фигуры. В этом окне должны находиться элементы графического интерфейса: выпадающий список, три ползунка и кнопка. С помощью выпадающего списка задаётся тип фигуры (круг, квадрат или треугольник), с помощью слайдеров задаётся цвет фигуры, при нажатии на кнопку, фигура рисуется на фоне в случайном месте. Рисуемая фигура должна также изображаться внутри окна, чтобы было видно, что мы будем рисовать. При изменении размера окна, размер элементов графического интерфейса должен меняться пропорционально.

\item \texttt{RegularWindow}, наследник классов \texttt{ResizableWindow} и \texttt{ClosableWindow}. Окно должно уметь закрываться и менять размер.
\end{itemize}
Для создания окон используйте класс контекстное меню, которое можно найти в файле \texttt{context\_menu.cpp}. В контекстном меню должно быть восемь пунктов: первые семь используются для создания окон одного из типов, использующихся в задании, восьмой пункт используется для удаления всех окон.
