...
Напишите функцию \texttt{isDomainName}, которая принимает на вход строку и если строка начинается на \texttt{www.} и заканчивается на \texttt{.com}, то эта функция должна вернуть \texttt{true}, иначе \texttt{false}. Используйте методы строки \texttt{starts\_with} и \texttt{ends\_with}.
\begin{lstlisting}
#include <iostream>
#include <string>
...
\newpage
\subsection{Парсинг по разделителю}
Напишите функцию \texttt{split}, которая принимает строку и символ-разделитель и возвращает \texttt{std::vector<std::string>}, содержащий все подстроки, разделенные этим символом. Используйте методы строки \texttt{find} и \texttt{substr}.
\begin{lstlisting}
#include <iostream>
#include <string>
#include <vector>
// Тут нужно написать функцию split
...
\subsection{Длина слов в файле}
Напишите функцию:
\begin{lstlisting}
std::vector<std::vector<std::size_t>> calculate_word_lengths(const std::string& filename)
\end{lstlisting}
которая будет считывать файл и считать количество символов в каждом слове файла. Под "словом"{} считается любая последовательность непробельных символов, ограниченная пробельными символами или началом/концом файла. Функция должна возвращать вектор, содержащий вектора количества символов слов для каждой строки. Например, если файл содержит:
\begin{lstlisting}
one two three
hello
axolotl bison cat dog eagle
\end{lstlisting} 
то функция должна вернуть вектор, содержащий:
\begin{lstlisting}
{{3, 3, 5}, {5}, {7, 5, 3, 3, 5}}
\end{lstlisting} 
