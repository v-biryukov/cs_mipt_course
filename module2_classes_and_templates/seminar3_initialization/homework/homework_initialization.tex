...
\subsection{Наличие особых методов}
\begin{itemize}
\item Класс является \textit{default constructible}, если у него есть доступный конструктор по умолчанию, который можно использовать для создания объекта.
\item Класс является \textit{copy constructible}, если у него есть доступный конструктор копирования, который можно использовать для создания копии объекта.
\item Класс является \textit{copy assignable}, если у него есть доступный оператор копирующего присваивания, который можно использовать для присваивания объектов.
\end{itemize}
...
\begin{enumerate}
\item \begin{Verbatim}[commandchars=\\\{\}]
\textcolor{blue}{struct} Alice
\{
    Alice() \{\}
    Alice(\textcolor{blue}{const} Alice& other) \{\}
    Alice& operator=(\textcolor{blue}{const} Alice& other) \{\textcolor{blue}{return} *this;\}
\};
\end{Verbatim}

\item \begin{Verbatim}[commandchars=\\\{\}]
\textcolor{blue}{struct} Bob
\{
    Bob(\textcolor{blue}{int} x) \{\}
\};
\end{Verbatim}

\item \begin{Verbatim}[commandchars=\\\{\}]
\textcolor{blue}{struct} Casper
\{
    Casper() = \textcolor{blue}{default};
    Casper(\textcolor{blue}{const} Casper& other) = \textcolor{blue}{delete};
    Casper& operator=(\textcolor{blue}{const} Casper& other) = \textcolor{blue}{default};
\};
\end{Verbatim}

\item \begin{Verbatim}[commandchars=\\\{\}]
\textcolor{blue}{struct} Diana \{\};
\end{Verbatim}
\end{enumerate}
Для того, чтобы сдать эту задачу нужно создать файл в формате \texttt{.txt} и, используя любой текстовый редактор, записать в него ответы в следующем формате (ответы ниже неверны):
\begin{verbatim}
...
\item Массив объектов типа \texttt{int}, содержащий элементы \texttt{10, 20, 30, 40, 50}.
\item Один объект типа \texttt{std::vector}, содержащий элементы \texttt{10, 20, 30, 40, 50}.
\item Массив объектов типа \texttt{std::string}, равный \texttt{\{"Cat"{}, "Dog"{}, "Mouse"\}}.
\end{itemize}
...
Создайте объекты класса \texttt{std::string} следующими способами:
\begin{itemize}
\item Создайте объект класса \texttt{std::string} на стеке обычным образом и инициализируйте его строкой \texttt{"Cat"}.
\item Создайте объект класса \texttt{std::string} в куче, используя оператор \texttt{new}, и инициализируйте его строкой \texttt{"Dog"}. Напечатайте этот объект на экран. Удалите этот объект с помощью оператора \texttt{delete}.
\item Пусть у нас есть массив:
\begin{lstlisting}
char x[sizeof(std::string)];
\end{lstlisting}
Создайте объект класса \texttt{std::string} в этом массиве с помощью оператора \texttt{placement new} и инициализируйте объект строкой \texttt{"Elephant"}. Напечатайте этот объект на экран. Корректно уничтожьте этот объект.
\end{itemize}

\subsection{Обёртка}
Напишите класс \texttt{RestrictWrapper}, который бы оборачивал переменную типа \texttt{int}. Этот класс должен иметь как минимум следующие поля:
\begin{enumerate}
\item Ссылку на переменную типа \texttt{int}.
\item Константу \texttt{minValue}.
\item Константу \texttt{maxValue}.
\end{enumerate}
и методы:
\begin{enumerate}
\item Конструктор, который задаёт поля. Если значение переменной, на которую указывает ссылка, не находится в диапазоне от \texttt{minValue} до \texttt{maxValue}, то её нужно изменить также, как это делается в методе \texttt{set}.
\item Метод \texttt{get}, который возвращает значение оборачиваемой переменной.
\item Метод \texttt{set}, который задаёт значение оборачиваемой переменной, если значение находится в диапазоне от \texttt{minValue} до \texttt{maxValue}. Если же в \texttt{set} передаётся значение меньше,  чем \texttt{minValue}, то переменная должна установиться в значение \texttt{minValue}. Если в \texttt{set} передаётся значение больше, чем \texttt{maxValue}, то переменная должна установиться в значение \texttt{maxValue}.
\item Метод \texttt{numberOfGets} который возвращает количество вызовов метода \texttt{get} этого объекта.
\item Метод \texttt{numberOfSets} который возвращает количество вызовов метода \texttt{set} этого объекта.
\end{enumerate}
...
int main()
{
	...
	std::cout << w.numberOfGets() << std::endl;	// Напечатает 3
	std::cout << w.numberOfSets() << std::endl;	// Напечатает 4
}
\end{lstlisting}
