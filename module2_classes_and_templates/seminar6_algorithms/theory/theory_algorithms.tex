...
\begin{document}
\title{Семинар \#6: Алгоритмы и функциональные объекты. \vspace{-5ex}}\date{}\maketitle
...
*it = 4;
\end{lstlisting}
Так как тип этого итератора может иметь длинное название, то была введена функция под названием\\ \texttt{std::back\_inserter}, которая принимает на вход контейнер и возвращает такой итератор. Пример в котором вектор \texttt{a} копируется в пустой вектор \texttt{b}:
\begin{lstlisting}
std::vector<int> a {10, 20, 30, 40, 50};
std::vector<int> b;
std::cout << b;
...
\subsection*{Итератор \texttt{std::istream\_iterator}}
\texttt{std::istream\_iterator<T>} -- это специальный итератор у которого операторы перегружены по другому:
\begin{itemize}
\item \texttt{operator*} -- получить текущее значение из входного потока
\item \texttt{operator++} -- перейти к следующему значению
\item \texttt{operator=} -- ничего не делает
...
\newpage
\section*{Создание своих шаблонных функций для алгоритмов}
...
\subsection*{Лямбда-функции: основы}
\subsection*{\texttt{std::function}}
\subsection*{Указатели на поля и методы, функция \texttt{std::mem\_fn}}
\begin{lstlisting}
#include <iostream>
#include <iterator>
#include <string>
...
int main()
{
    int Book::* p = &Book::year;
    Book a = {"Hello", 10, 20, 30};
    Book b = {"Goodbye", 100, 200, 300};
    Book* qa = &a;
    
    std::cout << a.*p << std::endl;
    std::cout << qa->*p << std::endl;
    
    
    void (Book::* pf)(int) = &Book::setPrice;
    (a.*pf)(123);
    
    auto f = std::mem_fn(&Book::setPrice);
    f(&a, 123);
}
\end{lstlisting}
...
\newpage
\section*{Создание своих шаблонных функций с функциональными объектами}

\newpage
\section*{Лямбда функции}
Захват переменных. Локальные и глобальные переменные. Захват по значению и по ссылке. Захват всего по значению / по ссылке. Висячие ссылки. Инициализация переменных внутри захвата. Захват \texttt{this}. 

Лямбды как функторы. mutable-лямбды. \texttt{auto} и лямбды.
