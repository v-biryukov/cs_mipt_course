...
Следующая программа содержит пять функций типа \texttt{int(int, int)}, а также массив \texttt{array}, который содержит адреса этих функций. \texttt{print\_func\_name} печатает название одной из данных функций по её адресу. Для удобства введёно новое имя типа \texttt{FuncPtr} для указателя типа \texttt{int(*)(int, int)}. Напишите функцию: 
\begin{lstlisting}
void sort_funcs(FuncPtr* a, size_t n, int x, int y)
\end{lstlisting}
которая будет сортировать массив \texttt{array} по возрастанию возвращаемого значения при аргументах \texttt{x} и \texttt{y}.
\begin{lstlisting}
#include <stdio.h>
#include <stdlib.h>
...
// Тут вам нужно написать функцию sort\_funcs
...
\end{lstlisting}
Можете использовать любой алгоритм сортировки: выбором, пузырьком, \texttt{qsort} или другой.
