...
\begin{itemize}
\item Напечатайте все байты через пробел. Используйте шестнадцатеричную систему счисления.
\item Напечатайте все байты через пробел. Используйте десятичную систему счисления.
\item Напечатайте все байты в следующем формате.
\begin{lstlisting}
Index: 04  Hex: 0x40  ASCII: @
\end{lstlisting}
Если для байта нет печатаемого символа в ASCII, то на месте символа напечатайте символ \texttt{.} (точка).
\item Присвойте байту с индексом 5 значение \texttt{0xE5}.
\item Скопируйте первые семь байт на три байта правее. То есть нужно скопировать участок памяти с индексами от 0 до 6 в участок памяти с индексами от 3 до 9. Используйте \texttt{memmove}.
\item Скопируйте последние пять байт в начало массива. Используйте \texttt{memcpy}.
\item Занулите байты с идексами от 5 до 10. Используйте \texttt{memset}.
...
\item \texttt{int b = 1000000000;}
\item \texttt{int c = 65535;}
\item \texttt{int d = -1;}
\item \texttt{int array[3] = \{10, 2000, 65535}\};
\item \texttt{char str[9] = "Cat\textbackslash nDog\textbackslash n"};
...
\subsection{Строка}
В программе создан массив из четырёх целых чисел типа \texttt{int}. В памяти этот массив занимает 16 байт. Интерпретируйте байты этого массива как строку и напечатайте её.
\begin{lstlisting} 
#include <stdio.h>
int main() 
{
...
\begin{enumerate}[(a)]
\item Используя приведение указателей.
\item Используя \texttt{union}.
\item Используя \texttt{memcpy}.
\end{enumerate}
...
\end{itemize}
Используйте \texttt{xxd}, чтобы просмотреть содержимое файлов.


\subsection{Последние символы}
...
~
\newpage
\section{Необязательные задачи (никак не учитываются)}
\subsection{Числа без указания количества}
В файле \texttt{numbers.txt} хранятся некоторые целые числа (но не указано их количество). Напишите программу, которая будет считывать все числа из этого файла и печатать их на экран. Если в файле содержится какие-то другие символы кроме цифр и пробельных символов, то программа должна печатать \texttt{Error!} и завершаться.\\
